/**
 * @class
 * Controller que implementa la busqueda y listado de Cargo no asignados.
 *
 * @name angular-keycloak-seed.controller##CargosNoAsignadosListCtrl
 * @author <a href = "mailto:federico.torres@konecta.com.py">Federico Torres</a>
 */

app.controller('CargosNoAsignadosListCtrl', ['$scope', 'CargosNoAsignadosService', 'LegajoService',
        'ProgramaService', 'CptService','CtsService', '$controller', '$rootScope',
    function ($scope, service, LegajoService, ProgramaService, CptService, 
            CtsService, $controller, $rootScope) {
        ...
        if ($rootScope.esHacienda) {
            ...
            var header = [{
                    ...
                    "key": "descripcionTipoPresupuesto",
                    visible: false
                }, {
                    "name": "Línea",
                    "key": "linea",
        ...
        (function initialize() {
            ...
            if ($scope.tienePermiso('ACTIVAR_CARGO')) {
                header.push({
                    "name": "Disponible",
                    "key": ""
                });
            ...
            service.cambiarEstado(cargo).then(
                function (response) {
                    ...
                    if(response.data){
                        ...
                        Message.error("El cargo tiene un CPT asignado, no se puede eliminar");
                    }

                })
                .catch(function (data) {
                    cargo.disponible = !cargo.disponible;
