 ...
 */
app.controller('BaseCptFormCtrl', ['$scope', '$routeParams', '$timeout', '$location',
    function ($scope, $routeParams, $timeout, $location) {
        ...
        $scope.crearRecurso = function () {
            console.log('Crear CPT : ', $scope.recurso.inicioVigencia, ' - ', $scope.recurso.finVigencia);
        ...
        $scope.getRecurso = function () {
            this.service.obtener($routeParams.id)
                .then(function (response) {
                    ...
                    if (!$scope.isCrear()) {
                        $scope.activo = $scope.recurso.activo;
        ...
        $scope.cambiarEstado = function (recurso) {
            ...
            if (window.confirm("¿Está seguro de " + operacion + " el Registro?"))
                this.service.actualizar(recurso).then(cambiarEstadoSuccess, cambiarEstadoError);
        };
